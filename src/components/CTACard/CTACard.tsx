  ...
  return (
    <div className="bg-white border-[3px] border-black p-8 md:p-10 flex flex-col items-start h-full hover:-translate-y-2 transition-transform duration-200">
      <div className="mb-8 h-20 w-auto">
        <img src={icon} alt={altText} className="h-full w-auto object-contain" />
      </div>
      ...
      {href ? (
        ...
        <button
          onClick={onClick}
          className="w-full text-center uppercase font-extrabold text-green-primary border-[3px] border-green-primary px-6 py-4 hover:bg-green-primary hover:text-white transition-colors text-lg tracking-wide cursor-pointer"
        >
          {textButton}
        </button>
      ) : (
